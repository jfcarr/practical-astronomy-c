...
THourAngle right_ascension_to_hour_angle(double ra_hours, double ra_minutes,
  ...
  int hour_angle_hours = ma_decimal_hours_hour(hour_angle);
  int hour_angle_minutes = ma_decimal_hours_minute(hour_angle);
  double hour_angle_seconds = ma_decimal_hours_second(hour_angle);
...
TRightAscension hour_angle_to_right_ascension(
  ...
  double right_ascension = ma_hour_angle_to_right_ascension(
  ...
  int right_ascension_hours = ma_decimal_hours_hour(right_ascension);
  int right_ascension_minutes = ma_decimal_hours_minute(right_ascension);
  int right_ascension_seconds = ma_decimal_hours_second(right_ascension);
...
THorizonCoordinates equatorial_coordinates_to_horizon_coordinates(
  ...
  double azimuth_in_decimal_degrees = ma_equatorial_coordinates_to_azimuth(
  ...
  double altitude_in_decimal_degrees = ma_equatorial_coordinates_to_altitude(
      hour_angle_hours, hour_angle_minutes, hour_angle_seconds,
      declination_degrees, declination_minutes, declination_seconds,
      geographical_latitude);

  double azimuth_degrees =
      ma_decimal_degrees_degrees(azimuth_in_decimal_degrees);
  double azimuth_minutes =
      ma_decimal_degrees_minutes(azimuth_in_decimal_degrees);
  double azimuth_seconds =
      ma_decimal_degrees_seconds(azimuth_in_decimal_degrees);

  double altitude_degrees =
      ma_decimal_degrees_degrees(altitude_in_decimal_degrees);
  double altitude_minutes =
      ma_decimal_degrees_minutes(altitude_in_decimal_degrees);
  double altitude_seconds =
      ma_decimal_degrees_seconds(altitude_in_decimal_degrees);
...
TEquatorialCoordinates horizon_coordinates_to_equatorial_coordinates(
    ...
    double geographical_latitude) {
  double hour_angle_in_decimal_degrees = ma_horizon_coordinates_to_hour_angle(
      azimuth_degrees, azimuth_minutes, azimuth_seconds, altitude_degrees,
      altitude_minutes, altitude_seconds, geographical_latitude);

  double declination_in_decimal_degrees = ma_horizon_coordinates_to_declination(
      azimuth_degrees, azimuth_minutes, azimuth_seconds, altitude_degrees,
      altitude_minutes, altitude_seconds, geographical_latitude);

  int hour_angle_hours = ma_decimal_hours_hour(hour_angle_in_decimal_degrees);
  int hour_angle_minutes =
      ma_decimal_hours_minute(hour_angle_in_decimal_degrees);
  double hour_angle_seconds =
      ma_decimal_hours_second(hour_angle_in_decimal_degrees);

  double declination_degrees =
      ma_decimal_degrees_degrees(declination_in_decimal_degrees);
  double declination_minutes =
      ma_decimal_degrees_minutes(declination_in_decimal_degrees);
  double declination_seconds =
      ma_decimal_degrees_seconds(declination_in_decimal_degrees);
...
double mean_obliquity_of_the_ecliptic(double greenwich_day, int greenwich_month,
                                      int greenwich_year) {
  double jd = ma_civil_date_to_julian_date(greenwich_day, greenwich_month,
                                           greenwich_year);
...
TEquatorialCoordinates2 ecliptic_coordinates_to_equatorial_coordinates(
  ...
  double ec_lon_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      ecliptic_longitude_degrees, ecliptic_longitude_minutes,
      ecliptic_longitude_seconds);
  double ec_lat_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
  ...
  double obliq_deg = ma_obliq(greenwich_day, greenwich_month, greenwich_year);
  ...
  double dec_deg = ma_degrees(dec_rad);
  double y =
      sin(ec_lon_rad) * cos(obliq_rad) - tan(ec_lat_rad) * sin(obliq_rad);
  double x = cos(ec_lon_rad);
  double ra_rad = atan2(y, x);
  double ra_deg1 = ma_degrees(ra_rad);
  double ra_deg2 = ra_deg1 - 360 * floor(ra_deg1 / 360);
  double ra_hours = ma_decimal_degrees_to_degree_hours(ra_deg2);

  int out_ra_hours = ma_decimal_hours_hour(ra_hours);
  int out_ra_minutes = ma_decimal_hours_minute(ra_hours);
  double out_ra_seconds = ma_decimal_hours_second(ra_hours);
  double out_dec_degrees = ma_decimal_degrees_degrees(dec_deg);
  double out_dec_minutes = ma_decimal_degrees_minutes(dec_deg);
  double out_dec_seconds = ma_decimal_degrees_seconds(dec_deg);
...
TEclipticCoordinates equatorial_coordinate_to_ecliptic_coordinate(
    ...
    int gw_year) {
  double ra_deg = ma_degree_hours_to_decimal_degrees(
      ma_hms_dh(ra_hours, ra_minutes, ra_seconds));
  double dec_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      dec_degrees, dec_minutes, dec_seconds);
  double ra_rad = degrees_to_radians(ra_deg);
  double dec_rad = degrees_to_radians(dec_deg);
  double obliq_deg = ma_obliq(gw_day, gw_month, gw_year);
  double obliq_rad = degrees_to_radians(obliq_deg);
  double sin_ecl_rad = sin(dec_rad) * cos(obliq_rad) -
                       cos(dec_rad) * sin(obliq_rad) * sin(ra_rad);
  double ecl_lat_rad = asin(sin_ecl_rad);
  double ecl_lat_deg = ma_degrees(ecl_lat_rad);
  double y = sin(ra_rad) * cos(obliq_rad) + tan(dec_rad) * sin(obliq_rad);
  double x = cos(ra_rad);
  double ecl_long_rad = atan2(y, x);
  double ecl_long_deg1 = ma_degrees(ecl_long_rad);
  double ecl_long_deg2 = ecl_long_deg1 - 360 * floor(ecl_long_deg1 / 360);

  double out_ecl_long_deg = ma_decimal_degrees_degrees(ecl_long_deg2);
  double out_ecl_long_min = ma_decimal_degrees_minutes(ecl_long_deg2);
  double out_ecl_long_sec = ma_decimal_degrees_seconds(ecl_long_deg2);
  double out_ecl_lat_deg = ma_decimal_degrees_degrees(ecl_lat_deg);
  double out_ecl_lat_min = ma_decimal_degrees_minutes(ecl_lat_deg);
  double out_ecl_lat_sec = ma_decimal_degrees_seconds(ecl_lat_deg);
...
TGalacticCoordinates equatorial_coordinate_to_galactic_coordinate(
  ...
  double b_deg = ma_degrees(b_radians);
  ...
  double long_deg1 = ma_degrees(atan2(y, x)) + 33;
  double long_deg2 = long_deg1 - 360 * floor(long_deg1 / 360);

  double gal_long_deg = ma_decimal_degrees_degrees(long_deg2);
  double gal_long_min = ma_decimal_degrees_minutes(long_deg2);
  double gal_long_sec = ma_decimal_degrees_seconds(long_deg2);
  double gal_lat_deg = ma_decimal_degrees_degrees(b_deg);
  double gal_lat_min = ma_decimal_degrees_minutes(b_deg);
  double gal_lat_sec = ma_decimal_degrees_seconds(b_deg);
...
TEquatorialCoordinates2 galactic_coordinates_to_equatorial_coordinates(
  ...
  double g_long_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      gal_long_deg, gal_long_min, gal_long_sec);
  double g_lat_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      gal_lat_deg, gal_lat_min, gal_lat_sec);
  ...
  double dec_deg = ma_degrees(dec_radians);
  ...
  double ra_deg1 = ma_degrees(atan2(y, x)) + 192.25;
  double ra_deg2 = ra_deg1 - 360 * floor(ra_deg1 / 360);
  double ra_hours1 = ma_decimal_degrees_to_degree_hours(ra_deg2);

  double ra_hours = ma_decimal_hours_hour(ra_hours1);
  double ra_minutes = ma_decimal_hours_minute(ra_hours1);
  double ra_seconds = ma_decimal_hours_second(ra_hours1);
  double dec_degrees = ma_decimal_degrees_degrees(dec_deg);
  double dec_minutes = ma_decimal_degrees_minutes(dec_deg);
  double dec_seconds = ma_decimal_degrees_seconds(dec_deg);
...
TAngle angle_between_two_objects(double ra_long1_hour_deg, double ra_long1_min,
  ...
  double dec_lat1_deg1 = ma_degrees_minutes_seconds_to_decimal_degrees(
  ...
  double dec_lat2_deg1 = ma_degrees_minutes_seconds_to_decimal_degrees(
  ...
  double d_deg = ma_degrees(d_rad);

  double angle_deg = ma_decimal_degrees_degrees(d_deg);
  double angle_min = ma_decimal_degrees_minutes(d_deg);
  double angle_sec = ma_decimal_degrees_seconds(d_deg);
...
TRiseSet rising_and_setting(double ra_hours, double ra_minutes,
  ...
  double ra_hours1 = ma_hms_dh(ra_hours, ra_minutes, ra_seconds);
  double dec_rad = degrees_to_radians(
      ma_degrees_minutes_seconds_to_decimal_degrees(dec_deg, dec_min, dec_sec));
  ...
  double h_hours = ma_decimal_degrees_to_degree_hours(ma_degrees(acos(cos_h)));
  double lst_rise_hours =
      (ra_hours1 - h_hours) - 24 * floor((ra_hours1 - h_hours) / 24);
  double lst_set_hours =
      (ra_hours1 + h_hours) - 24 * floor((ra_hours1 + h_hours) / 24);
  double a_deg = ma_degrees(
  ...
  double ut_rise_hours1 = ma_greenwich_sidereal_time_to_universal_time(
      ma_local_sidereal_time_to_greenwich_sidereal_time(lst_rise_hours, 0, 0,
                                                        geog_long_deg),
      0, 0, gw_date_day, gw_date_month, gw_date_year);
  double ut_set_hours1 = ma_greenwich_sidereal_time_to_universal_time(
      ma_local_sidereal_time_to_greenwich_sidereal_time(lst_set_hours, 0, 0,
                                                        geog_long_deg),
      0, 0, gw_date_day, gw_date_month, gw_date_year);
...
correct_for_precession(double ra_hour, double ra_minutes, double ra_seconds,
  ...
  double ra1_rad = degrees_to_radians(ma_degree_hours_to_decimal_degrees(
      ma_hms_dh(ra_hour, ra_minutes, ra_seconds)));
  double dec1_rad =
      degrees_to_radians(ma_degrees_minutes_seconds_to_decimal_degrees(
          dec_deg, dec_minutes, dec_seconds));
  double t_centuries =
      (ma_civil_date_to_julian_date(epoch1_day, epoch1_month, epoch1_year) -
       2415020) /
      36525;
  double m_sec = 3.07234 + (0.00186 * t_centuries);
  double n_arcsec = 20.0468 - (0.0085 * t_centuries);
  double n_years =
      (ma_civil_date_to_julian_date(epoch2_day, epoch2_month, epoch2_year) -
       ma_civil_date_to_julian_date(epoch1_day, epoch1_month, epoch1_year)) /
      365.25;
  double s1_hours =
      ((m_sec + (n_arcsec * sin(ra1_rad) * tan(dec1_rad) / 15)) * n_years) /
      3600;
  double ra2_hours = ma_hms_dh(ra_hour, ra_minutes, ra_seconds) + s1_hours;
  double s2_deg = (n_arcsec * cos(ra1_rad) * n_years) / 3600;
  double dec2_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
                        dec_deg, dec_minutes, dec_seconds) +
                    s2_deg;

  int corrected_ra_hour = ma_decimal_hours_hour(ra2_hours);
  int corrected_ra_minutes = ma_decimal_hours_minute(ra2_hours);
  double corrected_ra_seconds = ma_decimal_hours_second(ra2_hours);
  double corrected_dec_deg = ma_decimal_degrees_degrees(dec2_deg);
  double corrected_dec_minutes = ma_decimal_degrees_minutes(dec2_deg);
  double corrected_dec_seconds = ma_decimal_degrees_seconds(dec2_deg);
...
TNutation nutation_in_ecliptic_longitude_and_obliquity(double greenwich_day,
                                                       int greenwich_month,
                                                       int greenwich_year) {
  double jd_days = ma_civil_date_to_julian_date(greenwich_day, greenwich_month,
                                                greenwich_year);
...
correct_for_aberration(double ut_hour, double ut_minutes, double ut_seconds,
  ...
  double true_long_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      true_ecl_long_deg, true_ecl_long_min, true_ecl_long_sec);
  double true_lat_deg = ma_degrees_minutes_seconds_to_decimal_degrees(
      true_ecl_lat_deg, true_ecl_lat_min, true_ecl_lat_sec);
  double sun_true_long_deg = ma_sun_long(ut_hour, ut_minutes, ut_seconds, 0, 0,
  ...
  double apparent_ecl_long_deg = ma_decimal_degrees_degrees(apparent_long_deg);
  double apparent_ecl_long_min = ma_decimal_degrees_minutes(apparent_long_deg);
  double apparent_ecl_long_sec = ma_decimal_degrees_seconds(apparent_long_deg);
  double apparent_ecl_lat_deg = ma_decimal_degrees_degrees(apparent_lat_deg);
  double apparent_ecl_lat_min = ma_decimal_degrees_minutes(apparent_lat_deg);
  double apparent_ecl_lat_sec = ma_decimal_degrees_seconds(apparent_lat_deg);
...
TCorrectedRefraction atmospheric_refraction(
  ...
  double ha_hour = ma_right_ascension_to_hour_angle(
      ...
      geog_long_deg);
  double azimuth_deg = ma_equatorial_coordinates_to_azimuth(
      ha_hour, 0, 0, true_dec_deg, true_dec_min, true_dec_sec, geog_lat_deg);
  double altitude_deg = ma_equatorial_coordinates_to_altitude(
      ha_hour, 0, 0, true_dec_deg, true_dec_min, true_dec_sec, geog_lat_deg);
  double corrected_altitude_deg =
      ma_refract(altitude_deg, coordinate_type1, atmospheric_pressure_mbar,
                 atmospheric_temperature_celsius);

  double corrected_ha_hour = ma_horizon_coordinates_to_hour_angle(
      azimuth_deg, 0, 0, corrected_altitude_deg, 0, 0, geog_lat_deg);
  double corrected_ra_hour1 = ma_hour_angle_to_right_ascension(
      corrected_ha_hour, 0, 0, lct_hour, lct_min, lct_sec,
      daylight_saving_hours, timezone_hours, lcd_day, lcd_month, lcd_year,
      geog_long_deg);
  double corrected_dec_deg1 = ma_horizon_coordinates_to_declination(
      azimuth_deg, 0, 0, corrected_altitude_deg, 0, 0, geog_lat_deg);

  int corrected_ra_hour = ma_decimal_hours_hour(corrected_ra_hour1);
  int corrected_ra_min = ma_decimal_hours_minute(corrected_ra_hour1);
  double corrected_ra_sec = ma_decimal_hours_second(corrected_ra_hour1);
  double corrected_dec_deg = ma_decimal_degrees_degrees(corrected_dec_deg1);
  double corrected_dec_min = ma_decimal_degrees_minutes(corrected_dec_deg1);
  double corrected_dec_sec = ma_decimal_degrees_seconds(corrected_dec_deg1);
...
TCorrectedParallax corrections_for_geocentric_parallax(
  ...
  double corrected_ha_hours =
      ma_parallax_ha(ha_hours, 0, 0, dec_deg, dec_min, dec_sec, coordinate_type,
                     geog_lat_deg, height_m, equatorial_hor_parallax_deg);

  double corrected_ra_hours = ma_hour_angle_to_right_ascension(
      corrected_ha_hours, 0, 0, lct_hour, lct_min, lct_sec, daylight_saving,
      timezone_hours, lcd_day, lcd_month, lcd_year, geog_long_deg);

  double corrected_dec_deg1 = ma_parallax_dec(
      ha_hours, 0, 0, dec_deg, dec_min, dec_sec, coordinate_type, geog_lat_deg,
      height_m, equatorial_hor_parallax_deg);

  int c_ra_hour = ma_decimal_hours_hour(corrected_ra_hours);
  int c_ra_min = ma_decimal_hours_minute(corrected_ra_hours);
  double c_ra_sec = ma_decimal_hours_second(corrected_ra_hours);
  double c_dec_deg = ma_decimal_degrees_degrees(corrected_dec_deg1);
  double c_dec_min = ma_decimal_degrees_minutes(corrected_dec_deg1);
  double c_dec_sec = ma_decimal_degrees_seconds(corrected_dec_deg1);
...
heliographic_coordinates(double helio_position_angle_deg,
  ...
  double julian_date_days =
      ma_civil_date_to_julian_date(gwdate_day, gwdate_month, gwdate_year);
  double t_centuries = (julian_date_days - 2415020) / 36525;
  double long_asc_node_deg =
      ma_degrees_minutes_seconds_to_decimal_degrees(74, 22, 0) +
      (84 * t_centuries / 60);
  double sun_long_deg =
      ma_sun_long(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year);
  double y = sin(degrees_to_radians(long_asc_node_deg - sun_long_deg)) *
             cos(degrees_to_radians(
                 ma_degrees_minutes_seconds_to_decimal_degrees(7, 15, 0)));
  ...
  double theta1_rad = atan(
      ...
      tan(degrees_to_radians(ma_obliq(gwdate_day, gwdate_month, gwdate_year))));
  double theta2_rad =
      atan(-cos(degrees_to_radians(long_asc_node_deg - sun_long_deg)) *
           tan(degrees_to_radians(
               ma_degrees_minutes_seconds_to_decimal_degrees(7, 15, 0))));
  double p_deg = ma_degrees(theta1_rad + theta2_rad);
  double rho1_deg = helio_displacement_arcmin / 60;
  double rho_rad =
      asin(2 * rho1_deg /
           ma_sun_dia(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year)) -
...
TSelenographicSubEarthCoordinates selenographic_coordinates1(double gwdate_day,
  ...
  double geocentric_moon_long_deg =
      ma_moon_longitude(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year);
  double geocentric_moon_lat_rad = degrees_to_radians(
      ma_moon_latitude(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year));
  double inclination_rad = degrees_to_radians(
      ma_degrees_minutes_seconds_to_decimal_degrees(1, 32, 32.7));
  ...
  double sub_earth_lat_deg = ma_degrees(asin(sin_be));
  ...
  double obliquity_rad =
      degrees_to_radians(ma_obliq(gwdate_day, gwdate_month, gwdate_year));
  ...
  double c_deg = ma_degrees(c1_rad + c2_rad);
...
TSelenographicSubSolarCoordinates selenographic_coordinates2(double gwdate_day,
  ...
  double moon_equ_hor_parallax_arc_min =
      ma_moon_horizontal_parallax(0, 0, 0, 0, 0, gwdate_day, gwdate_month,
                                  gwdate_year) *
      60;
  double sun_earth_dist_au =
      ma_sun_dist(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year);
  double geocentric_moon_lat_rad = degrees_to_radians(
      ma_moon_latitude(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year));
  double geocentric_moon_long_deg =
      ma_moon_longitude(0, 0, 0, 0, 0, gwdate_day, gwdate_month, gwdate_year);
  ...
  double sub_solar_lat_deg = ma_degrees(asin(sin_bs));
