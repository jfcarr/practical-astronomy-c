...
void test_solar_eclipse_occurrence(double local_date_day, int local_date_month,
                                   int local_date_year, bool is_daylight_saving,
                                   int zone_correction_hours,
                                   TSolarEclipseOccurrence expected_result) {
  TSolarEclipseOccurrence actual_result = solar_eclipse_occurrence(
      local_date_day, local_date_month, local_date_year, is_daylight_saving,
      zone_correction_hours);

  printf("[Solar Eclipse Occurrence]\n");
  printf("\tExpected:\n");
  printf("\t\tDate is %d/%0.0f/%d\n", expected_result.event_date_month,
         expected_result.event_date_day, expected_result.event_date_year);
  printf("\t\tStatus is %d\n", expected_result.status);
...
void test_solar_eclipse_circumstances(
    double local_date_day, int local_date_month, int local_date_year,
    bool is_daylight_saving, int zone_correction_hours,
    double geog_longitude_deg, double geog_latitude_deg,
    TSolarEclipseCircumstances expected_result) {
  TSolarEclipseCircumstances actual_result = solar_eclipse_circumstances(
      local_date_day, local_date_month, local_date_year, is_daylight_saving,
      zone_correction_hours, geog_longitude_deg, geog_latitude_deg);

  printf("[Solar Eclipse Circumstances]\n");
  printf("\tExpected:\n");
  printf("\t\tCertain Date is %d/%0.0f/%d\n",
         expected_result.solar_eclipse_certain_date_month,
         expected_result.solar_eclipse_certain_date_day,
         expected_result.solar_eclipse_certain_date_year);
  printf("\t\tFirst Contact is %0.0f h %0.0f m\n",
         expected_result.ut_first_contact_hour,
         expected_result.ut_first_contact_minutes);
  printf("\t\tMid Eclipse is %0.0f h %0.0f m\n",
         expected_result.ut_mid_eclipse_hour,
         expected_result.ut_mid_eclipse_minutes);
  printf("\t\tLast Contact is %0.0f h %0.0f m\n",
         expected_result.ut_last_contact_hour,
         expected_result.ut_last_contact_minutes);
  printf("\t\tMagnitude is %0.3f\n", expected_result.eclipse_magnitude);
  printf("\tActual:\n");
  printf("\t\tCertain Date is %d/%0.0f/%d\n",
         actual_result.solar_eclipse_certain_date_month,
         actual_result.solar_eclipse_certain_date_day,
         actual_result.solar_eclipse_certain_date_year);
  printf("\t\tFirst Contact is %0.0f h %0.0f m\n",
         actual_result.ut_first_contact_hour,
         actual_result.ut_first_contact_minutes);
  printf("\t\tMid Eclipse is %0.0f h %0.0f m\n",
         actual_result.ut_mid_eclipse_hour,
         actual_result.ut_mid_eclipse_minutes);
  printf("\t\tLast Contact is %0.0f h %0.0f m\n",
         actual_result.ut_last_contact_hour,
         actual_result.ut_last_contact_minutes);
  printf("\t\tMagnitude is %0.3f\n", actual_result.eclipse_magnitude);

  assert(actual_result.eclipse_magnitude == expected_result.eclipse_magnitude);
  assert(actual_result.solar_eclipse_certain_date_day ==
         expected_result.solar_eclipse_certain_date_day);
  assert(actual_result.solar_eclipse_certain_date_month ==
         expected_result.solar_eclipse_certain_date_month);
  assert(actual_result.solar_eclipse_certain_date_year ==
         expected_result.solar_eclipse_certain_date_year);
  assert(actual_result.ut_first_contact_hour ==
         expected_result.ut_first_contact_hour);
  assert(actual_result.ut_first_contact_minutes ==
         expected_result.ut_first_contact_minutes);
  assert(actual_result.ut_last_contact_hour ==
         expected_result.ut_last_contact_hour);
  assert(actual_result.ut_last_contact_minutes ==
         expected_result.ut_last_contact_minutes);
  assert(actual_result.ut_mid_eclipse_hour ==
         expected_result.ut_mid_eclipse_hour);
  assert(actual_result.ut_mid_eclipse_minutes ==
